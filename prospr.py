...
# メイン処理
def main(args):
    # casp_domainsはprospr/pconf.py にあるbasedirの値を入れている
    # basedir = "/mnt/HDD/database/"
    # casp_domainsは一度計算し終わって結果が存在しているpklのリスト
    casp_domains = os.listdir(pconf.basedir + args.domain)
    # もしargs.domainについてpklがなかったら、
...
# build処理。まだpconf.pyのbasedirに登録されていないdomainについて計算を行い、結果を格納する処理（？）
def build(args):
    p = PsiblastManager()
    if hhdb_dl_present():
        if hhdb_db_present():
